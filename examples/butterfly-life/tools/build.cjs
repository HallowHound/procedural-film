...
//   node tools/build.cjs --fixtures      writes dist/butterfly-life-fixtures.html from tools/fixtures
...
function build({ fixtures = false, out = null, quiet = false, lenient = false } = {}) {
  ...
  const outFile = out ? C.resolveOut(out) : path.join(C.ROOT, 'dist', fixtures ? 'butterfly-life-fixtures.html' : 'butterfly-life.html');
  ...
  const title = fixtures ? 'butterfly-life (fixtures)' : 'The life of a monarch';
